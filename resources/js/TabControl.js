...
var TabControl = function (options) {
    ...
    this.RefreshSize = function () {
        ...
        var domHeight = this.DOM().height();
        var Control1pagRollCloth = this.Controls.GetAll()[0];
        var controList = Control1pagRollCloth.Controls.GetAll();
        Control1pagRollCloth.Width(domWidth);
        Control1pagRollCloth.Height(domHeight);
        for (var i = 0; i < controList.length; i++) {
            controList[i].DOM().width(domWidth);
            controList[i].DOM().height(domHeight);
    ...
    this.CurrentPpage = function () {
        ...
        if (arguments.length <= 0) {
            ...
            $('input[name=CurrentPpage]').val(count);
            //this.Update();
        }
    }
    this.Page = function () {
        ...
        if (arguments.length <= 0) {
            ...
            _currentPpage = count;
            console.log(_currentPpage + '------' + count);
            //this.Update();
        }
    }
    this.RollingEffect = function () {
        ...
        if (arguments.length <= 0) {
            ...
            var cons = _rollCloth.Controls.GetAll();
            if (count == 0) {
                for (var a = 0; a < cons.length; a++) {
                    var doma = cons[a].DOM();
                    doma.css("transform", "none");
                    cons[a].ZIndex(100 - a);
                    //if (a == 0) {
                    //    doma.css("transform", "translateY(" + doma.height() + "px)");
                    //}
                }
            }
            if (count == 1) {
    ...
    function changeImg(curIndex, con, type) {
        ...
        if (as.RollingEffect() == 1) {
            ...
            if (list[curIndex - 1]) {
                var dom = list[curIndex-1].DOM();
                var transformVal = '';
                if (as.ScrollDirection() == 0) {
                    transformVal = "translateY(" + dom.height() + "px)";
                }
                if (as.ScrollDirection() == 1) {
                    transformVal = "translateY(" + -dom.height() + "px)";
                }
                if (as.ScrollDirection() == 2) {
                    transformVal = "translateY(" + dom.height() + "px)";
                }
                if (as.ScrollDirection() == 3) {
                    transformVal = "translateX(" + -dom.width() + "px)";
                }
                if (as.ScrollDirection() == 4) {
                    transformVal = "translateX(" + dom.width() + "px)";
                }
                dom.css("transform", transformVal);
            } else {
                //var Bdom = list[list.length - 1].DOM();
                //Bdom.css("transform", "translateY(" + Bdom.height() + "px)");
                for (var i = 0; i < list.length; i++) {
                    var doma = list[i].DOM();
                    doma.css("transform", "none");
                }
        ...
        if (as.RollingEffect() == 2) {
            if (list[curIndex - 1]) {
                ...
                var Bdom = list[list.length - 1].DOM();
                Bdom.css("transform", "translateY(0px)");
            }
            setTimeout(function () {
                var dom = list[curIndex].DOM();
                dom.css("transform", "translateY(" + dom.height() + "px)");
            }, 1000);
        }
    }

    function DomTrans(dom, type) {
        dom.css('-webkit-backface-visibility', 'hidden');
        dom.css('-webkit-transform-style', 'preserve-3d');
        dom.css("transition", "all 1s");
        dom.css("transform", "translateY(" + dom.height() + "px)");
    }
    ...
    var AppendLIDOMToULDOM = function () {
        ...
        if (broadcastBox) {
           ...
           // var self = HtmlControl.New(com);
            ...
            //self.DOM().css('position', 'inirr     rial');
            ...
            //    if (lengs == 1) {
            //        var doom = d[0];
            //        $(doom).css("transform", "translateY(" + d.height() + "px)");
            //        //$(doom).css("transition", "all 1s");

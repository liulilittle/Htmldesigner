...
!function ($) {
    ...
    var calculateObjectValue = function (self, name, args, defaultValue) {
        ...
        if (args[1].length == 0 && args[3] && args[3] != "id") {
            return "&nbsp;";
        }
    ...
    BootstrapTable.prototype.initTable = function () {
        ...
        if (this.dataLen(this.options.data)) {
            return;
        }
    ...
    BootstrapTable.prototype.initHeader = function () {
        ...
        $.each(this.options.columns, function (i, column) {
            ...
            style += sprintf('min-width: %spx; ', column.checkbox || column.radio ? 36 : column.minwidth);
            style += sprintf('width: %spx; ', column.checkbox || column.radio ? 36 : column.width);
            style += sprintf('white-space: %s; ', column.checkbox || column.radio ? 36 : column.whitespace);
    ...
    BootstrapTable.prototype.initPagination = function () {
        ...
        if (this.options.nowPageSize) {
            this.options.pageSize = $.isFunction(this.options.nowPageSize) ? this.options.nowPageSize() : this.options.nowPageSize;
        }
        ...
        if (this.options.sidePagination !== 'server') {
            this.options.totalRows = this.dataLen(data);
        }
                ...
                that.$body.find('tr').each(function () {
                    ...
                    $td.find(el).off(name).on(name, function (e) {
                        var index = $tr.data('index'),
                            row = that.objValue(that.data, index),
    ...
    BootstrapTable.prototype.initServer = function (silent) {
        ...
        data = calculateObjectValue(this.options, this.options.queryParams, [params, 0], data);
        ...
        var tag = { tag: JSON.stringify(this.options.tagParams) };
        ...
        $.ajax({
            ...
            success: function (res) {
                res = calculateObjectValue(that.options, that.options.responseHandler, [res,0], res);

                var data = res.Tag;

                if (that.options.sidePagination === 'server') {
                    that.options.totalRows = res.Tag.TotalCount;
                    data = res.Tag.Tag;
                }
                that.load(data);
    ...
    BootstrapTable.prototype.refresh = function (params) {
        if (params && params.url) {
            this.options.tagParams = params.tagParams;
    ...
    BootstrapTable.prototype.dataLen = function (obj) {
        var count = 0;
        for (var key in obj) {
            count++;
        }
    ...
    BootstrapTable.prototype.objValue = function (obj, i) {
        var count = 0;
        for (var key in obj) {
            if (count == i) {
                return obj[key]
